...
chart = {
  ...
  const linksAllLookup = {};
  linksAll.forEach(obj => {
    linksAllLookup[`${obj.source},${obj.target}`] = obj.value;
  });
  ...
  const linksCombined = linksAllEmb.map(obj => {
      const key = `${obj.source},${obj.target}`;
      return {
          ...obj,
          value: linksAllLookup[key] ? (1 - deps_vs_emb) * obj.value + deps_vs_emb * linksAllLookup[key] : obj.value
      };
  });
  ...
  const links = linksAll.filter(function(link) {
